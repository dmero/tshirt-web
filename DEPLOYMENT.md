...
6. Create a private S3 bucket for product images in your AWS account. Give the Render service credentials scoped to that bucket's required read/write operations. Set `AWS_STORAGE_BUCKET_NAME`, `AWS_S3_REGION_NAME`, `AWS_ACCESS_KEY_ID`, and `AWS_SECRET_ACCESS_KEY`. Upload existing `media/products/` files with their existing `products/...` keys; database image paths must match. The application uses signed image URLs and does not require a public bucket. Render's local filesystem is not used for persistent uploads.
